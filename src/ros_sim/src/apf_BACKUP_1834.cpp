...
class APFPlanner : public nav_core::BaseLocalPlanner {
    ...
    public:
        ...
        bool computeVelocityCommands(geometry_msgs::Twist& cmd_vel){
            ...
            // Geser target menjauhi thermal obstacle (Fuzzy Safety Corridor)
            target_y += corridor_shift_;
...

            {
                ...
                if (!thermal_semantic_.empty()) {
                    ...
                    if (hot_count > 8.0) {
                        ...
                        // CLEARANCE BIAS: dorong lebih jauh dari obstacle saat koridor lapang
                        double clearance_bias = 0.0;
                        if (zone_center_ > 0.80) {
                            clearance_bias = std::min(0.25, 0.20 + 0.1 * closeness);
                        }

                        if (obstacle_side < 0.0) {
                            obstacle_side -= clearance_bias;
                        } else {
                            obstacle_side += clearance_bias;
                        }
            ...
            /*
             * =====================================
             * FUZZY SAFETY CORRIDOR
             * =====================================
             */
            corridor_shift_ = 0.0;
            if (hot_count > 8.0) {
                double thermal_confidence = std::min(1.0, hot_count / 100.0);

                if (last_clearance_ > 0.80 && thermal_confidence > 0.25) {
                    double shift_strength = 0.20 + 0.20 * closeness;
                    shift_strength = std::min(0.40, shift_strength);

                    if (obstacle_side < 0.0) {
                        corridor_shift_ = shift_strength;
                    } else {
                        corridor_shift_ = -shift_strength;
                    }

                    ROS_INFO_THROTTLE(1.0,
                        "FUZZY SAFETY CORRIDOR: shift=%.3f m (tconf=%.2f clear=%.2f)",
                        corridor_shift_, thermal_confidence, last_clearance_);
                }
            }
