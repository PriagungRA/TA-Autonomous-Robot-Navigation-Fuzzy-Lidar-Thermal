...
class APFPlanner : public nav_core::BaseLocalPlanner {
    private:
        ...
        // ====== [THERMAL GLOBAL PATH REFINEMENT] keep-out region untuk global plan ======
        // Ketika kucing terdeteksi, waypoint global yang masuk zona aman harus digeser
        // agar robot mendapat ruang minimum clearance sebelum APF local processing.
        double cat_refine_enable_ = true;  // enable/disable path refinement
        double cat_path_margin_   = 0.10;  // geseran waypoint supaya jauh dari kucing (m)
        
        // ====== [PATH REFINEMENT CACHING] ======
        // Cache refined plan sekali per goal untuk menghindari zig-zag oscillation
        bool path_refined_ = false;  // flag: sudah di-refine?
        std::vector<geometry_msgs::PoseStamped> refined_plan_;  // cache refined plan
        
        // ====== [CAT AVOID HYSTERESIS] ======
        // Zona active/inactive dengan hysteresis untuk smooth activation/deactivation
        bool cat_avoid_active_ = false;  // state saat ini: apakah sedang hindari kucing?
        double cat_avoid_hysteresis_ = 0.15;  // deadband (m) untuk on-off transisi
        ...
        double cat_safe_margin_ = 0.25; // batas aman tepi-robot ke tepi-kucing (m) [INCREASED]
        double cat_radius_      = 0.20; // radius kucing (m) [INCREASED]
        double cat_influence_   = 0.35; // zona pengaruh di luar batas aman (m)
        double cat_keep_gain_   = 1.5;  // kekuatan tolakan [REDUCED - global plan sudah aman]
        ...
        double corridor_shift_ = 0.0;  // Fuzzy Safety Corridor shift
        // [SAFETY-CORRIDOR] parameter geseran menjauhi kucing (tunable rosparam)
        double corridor_shift_base_  = 0.30; // geseran dasar (m)
        double corridor_shift_max_   = 0.45; // batas atas geseran (m)
        double corridor_shift_clear_ = 0.80; // clearance LiDAR min agar boleh geser (m)
        ...
        // REVISI: Loosen stuck detector untuk menghindari false positive oscillation
        // Thermal sekarang hanya semantic modifier (weak), bukan hard obstacle
        double stuck_time_threshold_ = 12.0; // seconds (was 6.0, now more tolerant)
        double stuck_distance_threshold_ = 0.01; // meters 

        // ====== [THERMAL GLOBAL PATH REFINEMENT] ======
        // Menggeser waypoint global yang masuk zona keep-out dari kucing.
        // HANYA shift waypoint yang DEKAT (d < safe_dist + margin_active).
        // Waypoint jauh tetap TIDAK digeser agar tidak oscillate.
        void refinePlanByCatKeepOut(std::vector<geometry_msgs::PoseStamped>& plan)
        {
            if(!cat_refine_enable_ || !cat_init_) return;  // hanya jika cat_init == true

            ros::Time now_t = ros::Time::now();
            bool cat_active = cat_init_ &&
                              (now_t - last_cat_time_).toSec() < cat_latch_time_;
            if(!cat_active) {
                cat_avoid_active_ = false;  // clear flag saat kucing hilang
                return;
            }

            double robot_radius = 0.32;  // lebar fisik robot setengah (m)
            double safe_dist = robot_radius + cat_radius_ + cat_safe_margin_;
            
            // ====== [HYSTERESIS] ======
            // Zona on-off dengan deadband untuk smooth transisi
            double margin_active = 0.20;  // shift mulai dari distance ini
            
            if(!cat_avoid_active_ && plan.size() > 0){
                // Check: ada waypoint dalam zona aktif?
                bool should_activate = false;
                for(size_t i = 0; i < plan.size(); ++i){
                    double dx = plan[i].pose.position.x - cat_world_x_;
                    double dy = plan[i].pose.position.y - cat_world_y_;
                    double dist = std::hypot(dx, dy);
                    if(dist < safe_dist + margin_active){
                        should_activate = true;
                        break;
                    }
                }
                if(should_activate) cat_avoid_active_ = true;
            }
            else if(cat_avoid_active_ && plan.size() > 0){
                // Check: SEMUA waypoint sudah jauh dari kucing?
                bool all_safe = true;
                for(size_t i = 0; i < plan.size(); ++i){
                    double dx = plan[i].pose.position.x - cat_world_x_;
                    double dy = plan[i].pose.position.y - cat_world_y_;
                    double dist = std::hypot(dx, dy);
                    if(dist < safe_dist + margin_active + cat_avoid_hysteresis_){
                        all_safe = false;
                        break;
                    }
                }
                if(all_safe) cat_avoid_active_ = false;
            }

            int count_shifted = 0;

            // Shift HANYA waypoint dekat kucing
            for(size_t i = 0; i < plan.size(); ++i){
                double wp_x = plan[i].pose.position.x;
                double wp_y = plan[i].pose.position.y;

                double dx = wp_x - cat_world_x_;
                double dy = wp_y - cat_world_y_;
                double dist_to_cat = std::hypot(dx, dy);

                // Threshold: shift hanya jika dekat
                double shift_threshold = safe_dist + margin_active;
                
                if(dist_to_cat < shift_threshold){
                    // Smooth interpolation: geser bertahap, bukan langsung
                    double t = (dist_to_cat < safe_dist) ? 1.0 : 
                               (shift_threshold - dist_to_cat) / margin_active;
                    t = std::min(1.0, std::max(0.0, t));  // clamp 0..1
                    
                    // Geser waypoint radial menjauhi kucing
                    double dir_x = (dist_to_cat > 1e-3) ? (dx / dist_to_cat) : 1.0;
                    double dir_y = (dist_to_cat > 1e-3) ? (dy / dist_to_cat) : 0.0;

                    // Target posisi: jarak = safe_dist + margin_active dari pusat kucing
                    double target_dist = safe_dist + margin_active;
                    double target_x = cat_world_x_ + target_dist * dir_x;
                    double target_y = cat_world_y_ + target_dist * dir_y;
                    
                    // Interpolasi: weighted blend original vs target
                    double shift_amount = t * 0.8;  // smooth factor (0.8 = 80% shift max)
                    double new_wp_x = wp_x + shift_amount * (target_x - wp_x);
                    double new_wp_y = wp_y + shift_amount * (target_y - wp_y);

                    plan[i].pose.position.x = new_wp_x;
                    plan[i].pose.position.y = new_wp_y;

                    count_shifted++;

                    ROS_DEBUG("PATH_REFINE: WP[%zu] shifted (%.2f,%.2f)->(%.2f,%.2f), d=%.2f, t=%.2f",
                        i, wp_x, wp_y, new_wp_x, new_wp_y, dist_to_cat, t);
                }
            }

            if(count_shifted > 0 || cat_avoid_active_){
                ROS_INFO_THROTTLE(1.0,
                    "[PATH_REFINE] %d wp shifted, avoid=%s (cat@(%.2f,%.2f) d_safe=%.2f)",
                    count_shifted, cat_avoid_active_ ? "ON" : "OFF", 
                    cat_world_x_, cat_world_y_, safe_dist);
            }
        }
    ...
    public:
        ...
        void initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros){
            if(!initialized_){
                ...
                private_nh.param("corridor_shift_base",  corridor_shift_base_,  corridor_shift_base_);
                private_nh.param("corridor_shift_max",   corridor_shift_max_,   corridor_shift_max_);
                private_nh.param("corridor_shift_clear", corridor_shift_clear_, corridor_shift_clear_);
        ...
        bool setPlan(const std::vector<geometry_msgs::PoseStamped>& plan){
            ...
            // ====== [PATH REFINEMENT RESET] ======
            // Reset cache saat goal baru diterima
            path_refined_ = false;
            refined_plan_.clear();
            cat_avoid_active_ = false;
        ...
        bool computeVelocityCommands(geometry_msgs::Twist& cmd_vel){
            ...
            // =====================================
            // THERMAL-AWARE GLOBAL PATH REFINEMENT (DISABLED)
            // =====================================
            // ROLLBACK: Disable PATH_REFINE karena redundan dengan thermal virtual obstacle
            // di costmap. Navfn sudah melihat virtual obstacle dan menghindarinya.
            // PATH_REFINE ditambahkan APF bergeser waypoint -> deadlock dengan costmap routing.
            // Solusi: Biarkan Navfn handle thermal routing, APF hanya LiDAR + human.
            /*
            if(cat_init_){
                if(!path_refined_){
                    refined_plan_ = transformed_plan;
                    refinePlanByCatKeepOut(refined_plan_);
                    path_refined_ = true;
                    ROS_WARN("[PATH_REFINE] Refined plan cached (size=%zu)", refined_plan_.size());
                }
                transformed_plan = refined_plan_;
            }
            else
            {
                refinePlanByCatKeepOut(transformed_plan);
            }
            */
...

            {
                ...
                if (!thermal_semantic_.empty()) {
                    ...
                    if (hot_count > 8.0) {
                        ...
                        // REVISI: Reduce thermal gain drastically (was 0.35, now capped at 0.30)
                        // Thermal sebagai semantic modifier, bukan hard obstacle
                        double strength = std::min(0.30, 0.12 + 0.25 * closeness);
                        strength *= 0.25; // Additional 0.25x reduction -> max 0.075 effective gain
                        ...
                        // REVISI: Add deadzone to filter noise
                        // Ignore thermal if magnitude too small (noise)
                        double thermal_mag = std::hypot(thermal_rep_x, thermal_rep_y);
                        if (thermal_mag < 0.01) {  // threshold 0.01 m/s
                            thermal_rep_x = 0.0;
                            thermal_rep_y = 0.0;
                        }
                    }
            ...
            // ========================================
            // [CAT KEEP-OUT] (DISABLED)
            // ========================================
            // ROLLBACK: Disable CAT KEEP-OUT APF force karena redundan.
            // Thermal obstacle sudah di costmap sebagai virtual_obstacle.
            // Costmap + Navfn handle routing, APF tidak perlu tolak kucing lagi.
            // 4-layer avoidance (thermal virtual + PATH_REFINE + CAT KEEP-OUT + APF)
            // menyebabkan deadlock. Solusi: hanya 1 layer (costmap) untuk kucing.
            double f_cat_x = 0.0;
            double f_cat_y = 0.0;
            /*
            // DISABLED CODE BELOW:
            {
                ...
                if(std::isfinite(brange) &&
                   (now_t - bstamp).toSec() < cat_fresh_time_){
                    double dd_to_center = brange + cat_radius_;
                ...
                if(cat_active && use_fuzzy_){
                    ...
                    if(d_cat < d0c){
                        double dd = d_cat - D_safe;
                        double f;
                        if(dd > 0.0){
                            double t = (d0c - d_cat) / cat_influence_;
                            if(t > 1.0) t = 1.0;
                            f = cat_keep_gain_ * t * t;
                        }else{
                            f = cat_keep_gain_ * (1.0 + (-dd) * 12.0);
                        }
            ...
            */
            ...
            if(current_state_ == NORMAL){
                ...
                // Total force calculation (second occurrence - same revisions)
                f_total_x = f_att_x + f_rep_x + f_cat_x;  // NO thermal_rep_x
                f_total_y = f_att_y + f_rep_y + thermal_rep_y + f_cat_y;  // thermal only Y
                ...
                if(curr_mag > 0.01 && prev_mag > 0.01){
                    ...
                    // REVISI: Loosen oscillation detection (3.0s -> 5.0s)
                    // With reduced thermal gain, minor oscillation is now acceptable
                    if(cos_theta < oscillation_threshold){
                        if((ros::Time::now() - last_avoidance_time_).toSec() > 5.0){
