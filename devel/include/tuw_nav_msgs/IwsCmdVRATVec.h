...
struct IwsCmdVRATVec_
{
  ...
  _v_type v;
   ...
   typedef std::vector<double, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<double>> _rho_type;
  _rho_type rho;

   typedef std::vector<double, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<double>> _phi_type;
  _phi_type phi;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::tuw_nav_msgs::IwsCmdVRATVec_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.rho.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.rho[i]);
    }
    if (v.rho.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.phi.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.phi[i]);
    }
    if (v.phi.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.delta_t.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.delta_t[i]);
    }
    if (v.delta_t.empty() || true)
      s << "]";
    if (true || !indent.empty())
