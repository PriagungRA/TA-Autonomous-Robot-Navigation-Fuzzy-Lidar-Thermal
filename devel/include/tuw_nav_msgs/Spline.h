...
struct Spline_
{
  ...
  _knots_type knots;
   ...
   typedef std::vector< ::tuw_nav_msgs::Float64Array_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::tuw_nav_msgs::Float64Array_<ContainerAllocator> >> _ctrls_type;
  _ctrls_type ctrls;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::tuw_nav_msgs::Spline_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.ctrls.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::tuw_nav_msgs::Float64Array_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.ctrls[i]);
    }
    if (v.ctrls.empty() || false)
      s << "]";
  }
