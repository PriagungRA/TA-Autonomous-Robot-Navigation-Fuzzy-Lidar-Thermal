...
struct Printer< ::tuw_multi_robot_msgs::RouterStatus_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::tuw_multi_robot_msgs::RouterStatus_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "missing_robots: ";
    if (v.missing_robots.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.missing_robots.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.missing_robots[i]);
    }
    if (v.missing_robots.empty() || true)
      s << "]";
    if (true || !indent.empty())
