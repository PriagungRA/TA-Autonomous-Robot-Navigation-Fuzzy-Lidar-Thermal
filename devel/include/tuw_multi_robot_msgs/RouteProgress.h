...
struct RouteProgress_
{
  ...
  _header_type header;
   ...
   typedef std::vector<int32_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<int32_t>> _passed_type;
  _passed_type passed;

   typedef int32_t _current_type;
  _current_type current;

   typedef std::vector<int32_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<int32_t>> _todo_type;
  _todo_type todo;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::tuw_multi_robot_msgs::RouteProgress_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.passed.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.passed[i]);
    }
    if (v.passed.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.todo.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.todo[i]);
    }
    if (v.todo.empty() || true)
      s << "]";
    if (true || !indent.empty())
