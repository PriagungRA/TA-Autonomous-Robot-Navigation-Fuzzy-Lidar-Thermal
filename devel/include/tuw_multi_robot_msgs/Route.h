...
struct Route_
{
  ...
  _header_type header;
   ...
   typedef std::vector< ::tuw_multi_robot_msgs::RouteSegment_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::tuw_multi_robot_msgs::RouteSegment_<ContainerAllocator> >> _segments_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::tuw_multi_robot_msgs::Route_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.segments.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::tuw_multi_robot_msgs::RouteSegment_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.segments[i]);
    }
    if (v.segments.empty() || false)
      s << "]";
  }
