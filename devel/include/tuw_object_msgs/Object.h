...
struct Object_
{
  ...
  _shape_type shape;
   ...
   typedef std::vector<double, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<double>> _shape_variables_type;
...
struct Printer< ::tuw_object_msgs::Object_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::tuw_object_msgs::Object_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "shape_variables: ";
    if (v.shape_variables.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.shape_variables.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.shape_variables[i]);
    }
    if (v.shape_variables.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "ids_confidence: ";
    if (v.ids_confidence.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.ids_confidence.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.ids_confidence[i]);
    }
    if (v.ids_confidence.empty() || true)
      s << "]";
    if (true || !indent.empty())
