...
struct AirskinColors_
{
  ...
  _header_type header;
   ...
   typedef std::vector<uint16_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<uint16_t>> _idx_type;
  _idx_type idx;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::tuw_airskin_msgs::AirskinColors_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.idx.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.idx[i]);
    }
    if (v.idx.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.colors.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::std_msgs::ColorRGBA_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.colors[i]);
    }
    if (v.colors.empty() || false)
      s << "]";
  }
