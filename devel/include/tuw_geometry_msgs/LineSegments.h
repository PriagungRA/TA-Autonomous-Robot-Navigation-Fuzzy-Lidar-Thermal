...
struct LineSegments_
{
  ...
  _header_type header;
   ...
   typedef std::vector< ::tuw_geometry_msgs::LineSegment_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::tuw_geometry_msgs::LineSegment_<ContainerAllocator> >> _segments_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::tuw_geometry_msgs::LineSegments_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.segments.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::tuw_geometry_msgs::LineSegment_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.segments[i]);
    }
    if (v.segments.empty() || false)
      s << "]";
  }
